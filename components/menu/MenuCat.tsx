...
const MenuCat = ({ setShowAlert, category }: { setShowAlert: any, category: string | string[] }) => {
    ...
    useEffect(() => {
        const getData = async () => {
            await axios.get(`${BASE_URL}/products/menu/cat/${category}`)
            .then(res => {
    ...
    return (
        <section id="menu" className="w-full h-full flex flex-col gap-5 bg-white pb-5">
            ...
            <h2 className="text-center font-bold text-sm uppercase">{category} {category === 'house' && 'special'}</h2>
