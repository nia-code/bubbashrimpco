...
const SignUp = (props: Props) => {
    ...
    const handleSubmit = async () => {
      ...
      if(!email || !password){
        ...
        setIsLoading(true)
     await axios.post(`${BASE_URL}/users/register`, {email, password, fullname})
      .then((user) => {
                ...
                <div className='h-screen w-full flex items-center justify-center'>
                  <div className='flex flex-col flex-1 items-center justify-center h-full lg:max-w-[30%] mx-auto w-full max-w-[90%]'>
                    <div className='w-full px-5 flex-col  flex gap-3 items-center'>
                      ...
                      <p className='text-sm font-semibold text-gray-500 mb-5 text-center'>Please enter your log in details</p>

                      {error && <p className='text-xs text-[#FD6B6B] text-center'>{error}</p>}
                      <div className='text-sm w-full  flex flex-col gap-2'>
                        <label className='font-semibold' htmlFor="email">Full Name</label>
                        <div className={`w-full px-5 py-5 rounded-md border ${error ? 'border-[#FD6B6B]' : 'border-[#6A6A6A]'}`}>
                          <input value={fullname} onChange={(e) => setFullname(e.target.value)} type="text" name='name' id='name' placeholder='Full Name ' className={`bg-transparent border-none outline-none w-full text-sm ${error ? 'text-[#FD6B6B]': 'text-black'}`} />
                        </div>
                      </div>
                      ...
                      <Link href='/auth/sign-in' className='text-blue-800 underline text-xs'>Already have an account? Sign In</Link>
                      <button disabled={isLoading} onClick={handleSubmit} type="button" className={`bg-yellow-700 hover:bg-yellow-800 ${isLoading && 'cursor-not-allowed'} text-white w-full py-5 rounded-2xl text-sm `}>
                          {isLoading ? <PropagateLoader size={5} color="#fff" className='py-2' />: 'Sign Up'}
                      </button>
